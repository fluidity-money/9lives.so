...
fn test_unpack_details() {
    ...
    let d = const_hex::decode("").unwrap();
    let (shares, invested, global_invested, winner) = unpack_details(&d).unwrap();
    assert_eq!(U256::ZERO, shares);
    assert_eq!(U256::ZERO, invested);
    assert_eq!(U256::from(2000000), global_invested);
    assert_eq!(fixed_bytes!("0000000000000000"), winner);
}
