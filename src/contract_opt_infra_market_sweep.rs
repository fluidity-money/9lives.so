    ...
    pub fn sweep(
        ...
        if !e.campaign_winner_set.get() {
            ...
            for winner in outcomes {
                let power = e.outcome_vested_power.get(winner);
                ...
                // amount.
                assert_or!(
                    voting_power_global >= e.campaign_global_power_vested.get(),
                ...
                // We need to check if the voting power is 0, and if it is, then we need to reset
                ...
                if !voting_power_winner.is_zero() {
                    ...
                    // the next nonce by flagging that we must begin the next calling period
        ...
        let victim_global_vested_power = e.user_global_vested_power.get(victim_addr);
        ...
        let victim_vested_arb_amt = e.user_global_vested_arb.get(victim_addr);
        ...
        let victim_correct_power_amt = e
            .user_outcome_vested_power
            .getter(campaign_winner)
            .get(victim_addr);
        let global_power_amt = e
        ...
        let pct_of_global_power_is_winner = e
            .outcome_vested_power
            .get(campaign_winner)
            .checked_mul(SCALING_FACTOR)
            .ok_or(Error::CheckedMulOverflow)?
            .checked_div(e.campaign_global_power_vested.get())
            .ok_or(Error::CheckedDivOverflow)?;
        //victim victed power * (1 - global power percent)
        let diluted_victim_power = e
            .user_global_vested_power
            .get(victim_addr)
            .checked_mul(SCALING_FACTOR)
            .ok_or(Error::CheckedMulOverflow)?
