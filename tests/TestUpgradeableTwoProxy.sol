...
contract TestUpgradeableTwoProxy is Test {
    ...
    function setUp() external {
        factoryProxy = IMockedFactory(
            address(
                new UpgradeableTwoProxy(
                    address(this),
    ...
    function testDelegatesOkAfterUpgrade() external {
        IProxyAdmin proxyAdmin = IProxyAdmin(
            address(
                uint160(uint256(vm.load(address(factoryProxy), bytes32(uint256(keccak256("eip1967.proxy.admin")) - 1))))
            )
        );
        proxyAdmin.upgrade(
        ...
        assertEq(
            MockImpl1_2(address(factoryProxy)).newTrading37E9F4BE(o, address(0), 0, 0, bytes32(0), address(0), false),
            9999
        );
    }
}
