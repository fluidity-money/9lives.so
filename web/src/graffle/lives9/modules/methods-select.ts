import * as $$SelectionSets from "./selection-sets";
import type * as $$Utilities from "graffle/utilities-for-generated";

//
//
//
//
//
//
// ==================================================================================================
//                                      Select Methods Interface
// ==================================================================================================
//
//
//
//
//
//

export interface $MethodsSelect {
  Query: Query;
  Mutation: Mutation;
  AssetMetadata: AssetMetadata;
  Pnl: Pnl;
  Asset: Asset;
  UnclaimedCampaign: UnclaimedCampaign;
  PriceEvent: PriceEvent;
  CommentInvestment: CommentInvestment;
  Comment: Comment;
  LP: LP;
  Settings: Settings;
  Profile: Profile;
  Claim: Claim;
  Position: Position;
  Campaign: Campaign;
  CampaignShare: CampaignShare;
  LeaderboardPosition: LeaderboardPosition;
  LeaderboardWeekly: LeaderboardWeekly;
  InvestmentAmounts: InvestmentAmounts;
  Outcome: Outcome;
  Wallet: Wallet;
  Share: Share;
  Changelog: Changelog;
  Activity: Activity;
  PriceMetadata: PriceMetadata;
}

//
//
//
//
//
//
// ==================================================================================================
//                                                Root
// ==================================================================================================
//
//
//
//
//
//

export interface Query {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Query>,
  ): $SelectionSet;
}

export interface Mutation {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Mutation>,
  ): $SelectionSet;
}

//
//
//
//
//
//
// ==================================================================================================
//                                            OutputObject
// ==================================================================================================
//
//
//
//
//
//

export interface AssetMetadata {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.AssetMetadata
    >,
  ): $SelectionSet;
}

export interface Pnl {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Pnl>,
  ): $SelectionSet;
}

export interface Asset {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Asset>,
  ): $SelectionSet;
}

export interface UnclaimedCampaign {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.UnclaimedCampaign
    >,
  ): $SelectionSet;
}

export interface PriceEvent {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.PriceEvent>,
  ): $SelectionSet;
}

export interface CommentInvestment {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.CommentInvestment
    >,
  ): $SelectionSet;
}

export interface Comment {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Comment>,
  ): $SelectionSet;
}

export interface LP {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.LP>,
  ): $SelectionSet;
}

export interface Settings {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Settings>,
  ): $SelectionSet;
}

export interface Profile {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Profile>,
  ): $SelectionSet;
}

export interface Claim {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Claim>,
  ): $SelectionSet;
}

export interface Position {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Position>,
  ): $SelectionSet;
}

export interface Campaign {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Campaign>,
  ): $SelectionSet;
}

export interface CampaignShare {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.CampaignShare
    >,
  ): $SelectionSet;
}

export interface LeaderboardPosition {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.LeaderboardPosition
    >,
  ): $SelectionSet;
}

export interface LeaderboardWeekly {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.LeaderboardWeekly
    >,
  ): $SelectionSet;
}

export interface InvestmentAmounts {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.InvestmentAmounts
    >,
  ): $SelectionSet;
}

export interface Outcome {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Outcome>,
  ): $SelectionSet;
}

export interface Wallet {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Wallet>,
  ): $SelectionSet;
}

export interface Share {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Share>,
  ): $SelectionSet;
}

export interface Changelog {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Changelog>,
  ): $SelectionSet;
}

export interface Activity {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<$SelectionSet, $$SelectionSets.Activity>,
  ): $SelectionSet;
}

export interface PriceMetadata {
  <$SelectionSet>(
    selectionSet: $$Utilities.Exact<
      $SelectionSet,
      $$SelectionSets.PriceMetadata
    >,
  ): $SelectionSet;
}

//
//
//
//
//
//
// ==================================================================================================
//                                               Union
// ==================================================================================================
//
//
//
//
//
//

//
//
//
//
//
//
// ==================================================================================================
//                                             Interface
// ==================================================================================================
//
//
//
//
//
//
