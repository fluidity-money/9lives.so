...
if (!thirdwebValidation.success) {
    console.error("Invalid thirdwen config variables: ", thirdwebValidation.error.name);
    throw new Error(thirdwebValidation.error.message);
}

const thirdweb = { ...thirdwebValidation.data as ThirdwebSchemaType & { chain: typeof superpositionTestnet, accountAbstraction: { sponsorGas: boolean, chain: typeof superpositionTestnet } }, client: thirdwebClient }
